...
public class UserControllerTest {
  ...
  public void postLoginWithValidCredentials() throws Exception {
    ...
    mockMvc.perform(post("/login")
        .contentType(contentType).content("{\"username\": \"Bond\", \"password\": \"password123\"}"))
        .andExpect(status().isOk());
  }
  ...
  public void postLoginWithMissingUsername() throws Exception {
    ...
    mockMvc.perform(post("/login")
        .contentType(contentType).content("{\"username\":, \"password\": \"password123\"}"))
  ...
  public void postLoginWithWrongPassword() throws Exception {
    when(userService.loginResponse("Bond", "wrongpassword")).
        thenReturn(new ResponseEntity<>(new User("Bond", "password123"), HttpStatus.BAD_REQUEST));
    mockMvc.perform(post("/login")
        .contentType(contentType).content("{\"username\": \"Bond\", \"password\": \"wrongpassword\"}"))
        .andExpect(status().isBadRequest());
  }
